...
// const listCategories = ref(props.categories)
const textKategori = ref('')
// const dataKategori = computed(() => textKategori.value && !listCategories.value.find(k => k.name == textKategori.value) ? [...listCategories.value, { id: textKategori.value, name: `Tambah "${textKategori.value}"` }] : [...listCategories.value])
// const addKategori = (val: Category) => {
//   if (val.name.includes('Tambah "')) {
//     const newKategori = { ...val, id: Math.max(...props.categories.map(c => c.id)) + 1 }
//     form.category = newKategori
...
const addCategory = () => {
  ...
  const newCategory = {
    id: strCategory.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/[\s-]+/g, "-").replace(/^-+|-+$/g, ""),
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-4">
      <h2 class="text-2xl font-semibold">Tambah Menu Baru</h2>
      <form class="mt-6 grid grid-cols-6 gap-4"
        @submit.prevent="form.transform((data) => ({ ...data, category: data.category.id })).post(route('admin.menu.store'))">
        <div class="col-span-6 sm:col-span-3 lg:col-span-2">
          <Label>Nama Menu<span class="text-red-600" title="Wajib diisi">*</span></Label>
          <Input class="mt-2" placeholder="Nama menu baru" v-model="form.name" />
